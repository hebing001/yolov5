...
class Conv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):  # act 是可以自定义最后一层的激活函数
        """Initializes a standard convolution layer with optional batch normalization and activation."""
        super().__init__()
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p, d), groups=g, dilation=d, bias=False)#参数个数 c2* c1*k*k
        self.bn = nn.BatchNorm2d(c2) #参数个数：c2*2
