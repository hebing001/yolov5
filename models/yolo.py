...
class Detect(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference 推理部分 终于找到了嘿嘿嘿嘿 我日了狗了
                ...
                if isinstance(self, Segment):  # (boxes + masks)
                    ...
                    xy, wh, conf = x[i].sigmoid().split((2, 2, self.nc + 1), 4)  #分割成3 个部分  batch_size, 3, 20, 20, 85
                    xy = (xy * 2 + self.grid[i]) * self.stride[i]  # xy中心点位置在那里
